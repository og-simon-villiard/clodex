...
const PINNED_NAMES = [
  ...
  // group-header "+" modal.
  'ticketDetect', 'ticketView', 'ticketTransition', 'ticketComment', 'ticketUrl', 'ticketBranchName',
  'markSessionTicket',
];
...
test('contract covers exactly the pinned 218-method surface', () => {
  assert.equal(PINNED_NAMES.length, 218, 'pinned list is the full 218-method surface');
...
test('preload builds exactly the pinned window.api surface by looping the table', () => {
  ...
  try {
    ...
    assert.equal(generated.length, 218, 'window.api has exactly 218 methods');
